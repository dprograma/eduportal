        ...
        <div>
            ...
            <?php foreach ($posts as $post): ?>
                <div class="edit-post-container">
                    <form class="card px-3 bg-none edit-post-form" id="editPost<?= $post->id ?>"
                        enctype="multipart/form-data" method="POST">
                        <div class="form-group mb-3 exam-div">
                            <label for="title">Title</label>
                            <input type="text" class="form-control" value="<?= htmlspecialchars($post->title) ?>" id="title"
                                name="title">
                        </div>

                        <div class="form-group mb-3">
                            <label for="category">Subject</label>
                            <input type="text" class="form-control" value="<?= htmlspecialchars($post->category) ?>"
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control" id="body" rows="3"
                                name="body"><?= htmlspecialchars($post->body) ?></textarea>
                        </div>
                        <div class="form-group mb-3 text-center">
                            <label class="custom-file">
                                <img src="<?= htmlspecialchars($post->img) ?>" class="img-fluid text-center"
                                    style="width:100px" alt="...">
                                <input type="file" name="upload" id="" placeholder="" class="custom-file-input"
                                    aria-describedby="fileHelpId">
                                <span class="custom-file-control"></span>
                            </label>
                            <small id="fileHelpId"
                                class="form-text text-white w-25 m-auto btn-primary cursor-pointer">Select image</small>
