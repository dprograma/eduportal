...
if (!empty(Session::get('loggedin'))) {
    ...
    $questions = toJson($pdo->select("SELECT `d`.*, `u`.fullname FROM `document` `d`, `users` `u` WHERE `d`.`user_id`= `u`.`id` AND `d`.`document_type` = 'past question' AND `d`.`user_id` IN (SELECT `id` FROM `users` WHERE `is_affiliate` = ?) AND `u`.`fullname` LIKE '%$full_name%' AND `d`.`exam_body` LIKE '%$subject%' AND `d`.`year` LIKE '%$year%' AND `d`.`published` LIKE '%$status%' LIMIT $limit OFFSET $offset", [1])->fetchAll(PDO::FETCH_ASSOC));

    $ebooks = toJson($pdo->select("SELECT `d`.*, `u`.fullname FROM `document` `d`, `users` `u` WHERE `d`.`user_id`= `u`.`id` AND `d`.`document_type` = 'ebook' AND `d`.`user_id` IN (SELECT `id` FROM `users` WHERE `is_affiliate` = ?) AND `u`.`fullname` LIKE '%$full_name%' AND `d`.`exam_body` LIKE '%$subject%' AND `d`.`year` LIKE '%$year%' AND `d`.`published` LIKE '%$status%' LIMIT $limit OFFSET $offset", [1])->fetchAll(PDO::FETCH_ASSOC));

    $publications = toJson($pdo->select("SELECT `d`.*, `u`.fullname FROM `document` `d`, `users` `u` WHERE `d`.`user_id`= `u`.`id` AND `d`.`document_type` = 'publication' AND `d`.`user_id` IN (SELECT `id` FROM `users` WHERE `is_affiliate` = ?) AND `u`.`fullname` LIKE '%$full_name%' AND `d`.`exam_body` LIKE '%$subject%' AND `d`.`year` LIKE '%$year%' AND `d`.`published` LIKE '%$status%' LIMIT $limit OFFSET $offset", [1])->fetchAll(PDO::FETCH_ASSOC));

    $search = "SELECT `d`.*, `u`.fullname FROM `document` `d`, `users` `u` WHERE `d`.`user_id`= `u`.`id` AND `d`.`user_id` IN (SELECT `id` FROM `users` WHERE `is_affiliate` = 1) AND `u`.`fullname` LIKE '%$full_name%' AND `d`.`exam_body` LIKE '%$subject%' AND `d`.`year` LIKE '%$year%' AND `d`.`published` LIKE '%$status%' LIMIT $limit";
    ...
    if (isset($_GET['id'])) {
        ...
        redirect('view-affiliate-past-questions', $successMessage, 'success');
        exit();
    }
...
require_once 'view/loggedin/agent/view-affiliate-documents.php';